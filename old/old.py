...
class TTscrap:  # object to scrap timetable data from HTML file
    ...
    def get_data(self):
        ...
        data2 = list(
            filter(None, [rows[b].find_all("td") for b in range(len(rows)) if b])
        )[
            :-1
        ]  # remove the last <tr> which has total credits
        for b in range(len(data2)):  # extracting only required data
            try:
                ...
                output += [[paras[c][0].text + "" + paras[c][1].text for c in range(3)]]
...
def ppschedule_get(data):
    morning_template = [
        ...
        ["MON", "A1 L1", "F1 L2", "D1 L3", "TB1 L4", "TG1 L5", "S11 L6"],
        ["TUE", "B1 L7", "G1 L8", "E1 L9", "TC1 L10", "TAA1 L11", "L12"],
        ["WED", "C1 L13", "A1 L14", "F1 L15", "TD1 L16", "TBB1 L17", "L18"],
        ["THU", "D1 L19", "B1 L20", "G1 L21", "TE1 L22", "TCC1 L23", "L24"],
        ["FRI", "E1 L25", "C1 L26", "TA1 L27", "TF1 L28", "TDD1 L29", "S15 L30"],
        ["SAT", "X11 L71", "X12 L72", "Y11 L73", "Y12 L74", "S8 L75", "L76"],
        ["SUN", "Y11 L83", "Y12 L84", "X11 L85", "X12 L86", "S10 L87", "L88"],
    ]
    afternoon_template = [
        [
            "Start\nEnd",
        ...
        ["MON", "A2 L31", "F2 L32", "D2 L33", "TB2 L34", "TG2 L35", "S3 L36"],
        ["TUE", "B2 L37", "G2 L38", "E2 L39", "TC2 L40", "TAA2 L41", "S1 L42"],
        ["WED", "C2 L43", "A2 L44", "F2 L45", "TD2 L46", "TBB2 L47", "S4 L48"],
        ["THU", "D2 L49", "B2 L50", "G2 L51", "TE2 L52", "TCC2 L53", "S2 L54"],
        ["FRI", "E2 L55", "C2 L56", "TA2 L57", "TF2 L58", "TDD2 L59", "L60"],
        ["SAT", "X21 L77", "Z21 L78", "Y21 L79", "W21 L80", "W22 L81", "L82"],
        ["SUN", "Y21 L89", "Z21 L90", "X21 L91", "W21 L92", "W22 L93", "L94"],
    ]
    ...
    table1 = tabulate(
        morning_template[1:], tablefmt="pretty", headers=morning_template[0]
    )
...
async def on_ready():
    ...
    cmds = {
        ...
        "countmein": [
            f"CountMeIn is a feature in {client.user.mention} which is used to keep track of events happening in VIT-C Server. Events are Announced by Admins/Mods and intrested members can register using this command.",
            f"```\ncountmein stats\ncountmein <Event Name>```\n",
            [
                [
                    "countmein stats",
                    "Retrives List of Ongoing Events and Registered Members",
                ],
                [
                    "countmein <Event Name>",
                    "Registers Yourself for an Ongoing Event. To Backof From an Event Enter this Command Again.",
                ],
            ],
        ],
...
async def help(ctx, *, cmd=""):
    if not cmd:
        ...
        if type(cmds[cmd]) == list:
            emb = Embed(
                description=cmds[cmd][0],
                title=cmd,
                colour=discord.Colour.from_rgb(207, 68, 119),
            )
            emb.add_field(name="Available Commands", value=cmds[cmd][1], inline=False)
    ...
    if contents[0] == "reset" and mod in auth.roles and len(contents) == 2:
        if contents[1] in data["countmein"]:
            temp_msg = await ctx.send(
                f"Entry For {contents[1]} Will Be Erased. Do you wanna proceed?"
            )
            tick = "☑️"
            x = "❌"
            ...
            await ctx.send(
                f"Man No count for `{contents[0]}` is happening. Getouttahere!"
            )
        ...
        if mod in auth.roles:
            if contents[1] in data["countmein"]:
                temp_msg = await ctx.send(
                    f"Count for {contents[1]} is ongoing. Do you want to Reset it?"
                )
                tick = "☑️"
                x = "❌"
                await temp_msg.add_reaction(tick)
                ...
                try:
                    ...
                    await temp_msg.edit(content="Stats Unchanged.")
                    return
            data["countmein"][contents[1]] = [
                [],
                auth.id,
                datetime.now(timezone("Asia/Kolkata")).strftime("%m/%d/%Y, %H:%M"),
            ]
        ...
        if contents[0] in data["countmein"]:
            if auth.id in data["countmein"][contents[0]][0]:
                temp_msg = await ctx.send(
                    "You Have already been registered. Do You wanna Deregister?"
                )
                tick = "☑️"
                x = "❌"
                await temp_msg.add_reaction(tick)
...
async def on_message(msg):
    ...
    if str(msg.channel.type) == "private" and msg.author.bot == False:
        ...
        if msg.channel.id in data["staffchat"]:
            ...
            chnl = await helpchnl.create_text_channel(
                name=f"{msg.author.name}-{msg.author.discriminator}", category=opendm
            )
        ...
        if msg.attachments:
            ...
            await webhook.send(
                ...
                avatar_url=msg.author.avatar_url_as(),
            )
            ...
            if data["staffchat"][a] == msg.channel.id:
                ...
                if "closedm" in msg.content and client.user in msg.mentions:
                    if data["staffchatdmstats"][a] == "closed":
                        await msg.channel.send(
                            embed=Embed(
                                description="DM is Already Closed",
                ...
                if "opendm" in msg.content and client.user in msg.mentions:
                    if data["staffchatdmstats"][a] == "open":
                        await msg.channel.send(
                            embed=Embed(
                                description="DM is Already Open",
                                color=discord.Color.random(),
                            )
                        )
                        return
                ...
                if data["staffchatdmstats"][a] == "first":
                    await chnl.send(
                        embed=Embed(
                            description=f"You are now texting with {msg.author.mention}",
                            color=discord.Color.random(),
                        )
                    )
                    ...
                    await msg.channel.send(
                        embed=Embed(
                            description=f"This DM is Closed. Use '{client.user.mention} opendm' to Open DM.",
                            color=discord.Color.random(),
                        )
                    )
                    return
    ...
    try:
        if (
            msg.content.split()[1].lower() in data["gifs"]
            and client.user in msg.mentions
        ):
    ...
    if initated:  # Delete Links in Chat And Warn
        ...
        for a in lst:
            if (
                a in msg.content
                and msg.channel != del_chn
                and mod not in msg.author.roles
            ):
                await msg.delete()
                await msg.channel.send(
                    f"LOL Refrain From Sending Links :)\n\n{msg.author.mention}"
                )
                break
...
async def addgif(ctx, name, gif):
    ...
    if name in commands:
        await ctx.send(
            f"`{name}` is a Command Used by the Bot. Try Again With a Different Keyword"
        )
        return
    elif name in data["gifs"] and mod not in author.roles:
        await ctx.send(
            f"Keyword `{name}` is in use. Request a Moderator to change the gif for the Designated keyword."
        )
        return
    if "https://" in gif and "gif" in gif:
        data["gifs"][name.lower()] = [gif, ctx.author.name]
        embed = Embed(
            description="Gif Has Been Successfully Added",
            colour=discord.Colour.from_rgb(207, 68, 119),
        )
        ...
        await logs_chn.send(
            embed=Embed(
                description=f"{ctx.author.mention} added a GIF to a keyword `{name}`"
            )
        )
    else:
...
async def removegif(ctx, name):
    ...
    if name.lower() in data["gifs"]:
        if mod in ctx.author.roles:
            ...
            await ctx.send(
                f"You don't have the Permission to remove GIFs. Request a Moderator For Assistance."
            )
    else:
...
async def uploadschedule(ctx):
    ...
    if id in data["scheduledata"]:
        temp_msg = await ctx.channel.send(
            f"Schedule for {ctx.message.author.mention} Already Found In DataBase. Do You Want To Update it?"
        )
        tick = "☑️"
        x = "❌"
        await temp_msg.add_reaction(tick)
    ...
    if ctx.message.attachments == []:
        ...
        emb.set_image(url="https://media.giphy.com/media/iUn97XJCrPO9a5KF1w/giphy.gif")
    ...
    if retrived_data[0] == "Wrong Format":
        await ctx.channel.send(
            f"{ctx.author.mention} The File Uploaded wasn't of Supported Format. If the problem Persists ping {aro.mention} with the following error.\n\n```\n{retrived_data[1]}\n```"
        )
        return
    try:
        ...
        await ctx.channel.send(
            f"{ctx.author.mention} Schedule Has Been Succesfully Updated. Debug info- {id}"
        )
        ...
        await logs_chn.send(
            embed=Embed(description=f"{ctx.author.mention} Uploaded their schedule")
        )
    except:
        await ctx.channel.send(
            f"{ctx.author.mention} Schedule Has Been Succesfully Updated. Debug info- {id}"
...
async def deleteschedule(ctx, user):
    ...
    if len(ctx.message.mentions) >= 2:
        if mod not in ctx.author.roles:
            ...
            temp_msg = await ctx.channel.send(
                f"Schedule for {ctx.message.mentions[1].mention} will be erased from our DataBase. Do You Want To proceed?"
            )
            tick = "☑️"
            x = "❌"
            await temp_msg.add_reaction(tick)
            ...
            await logs_chn.send(
                embed=Embed(
                    description=f"{ctx.message.mentions[1].mention} was erased from the database."
                )
            )
            return

    temp_msg = await ctx.channel.send(
        f"Schedule for {ctx.author.mention} will be erased from our DataBase. Do You Want To proceed?"
    )
    tick = "☑️"
    x = "❌"
    await temp_msg.add_reaction(tick)
    ...
    del data["scheduledata"][id]
    await logs_chn.send(
        embed=Embed(description=f"{ctx.author.mention} was erased from the database.")
    )
...
async def scanschedule(ctx, user=""):
    ...
    if id not in data["scheduledata"]:
        await loadgif.edit(
            content=f"Schedule For {member.mention} Not Found. Upload Your Schedule Using '/uploadschedule' command"
        )
        return
    auth_data = [
        (a[0].split(" - ")[0], a[2].strip())
        for a in data["scheduledata"][id]
        if a[0].split(" - ")[0] != "BMEE103N"
    ]
    # print(auth_data)
    mates = {}
...
async def listschedules(ctx):
    await ctx.send(
        embed=Embed(
            title="Members who had uploaded their Schedule.",
            description="\n".join(
                [client.get_user(a).mention for a in data["scheduledata"]]
            ),
        )
    )
...
async def weekschedule(ctx, *person):
    ...
    if id in data["scheduledata"]:
        ...
        if temp[0] != "Wrong Format":
            ...
            aro = await ctx.guild.fetch_member(608276451074113539)
            await ctx.channel.send(
                f"An error has occured While Retrieving your Data. Contact {aro.mention} with The following Time.\n\n`{dt.now()}`"
            )
    else:
        await loadmsg.delete()
        await loadgif.delete()
        await ctx.channel.send(
            f"Schedule For {member.mention} Not Found. Upload Your Schedule Using '{client.user.mention} uploadschedule' command"
        )
...
async def dayschedule(ctx, *content):
    ...
    if temp[0] != "Wrong Format":
        ...
        txt += f"Keys:-\n\n{table3}"
        loadmsg = await ctx.send("Loading....")
        loadgif = await ctx.send(
            "https://tenor.com/view/rikka-rolling-hand-gif-5040472"
        )
        font = ImageFont.truetype("CascadiaCode.ttf", 20)
        h, w = font.getsize_multiline(txt, spacing=5)
        difference = 80
...
async def on_raw_message_delete(msg):
    ...
    if del_chn != None:
        ...
        if (msg.cached_message != None) and (
            (msg.cached_message.author == client.user)
            or (msg.cached_message.content[:6] == "#s1art")
            or msg.cached_message.channel in (chnl, chnl2)
        ):
            return
...
@slash.context_menu(
    target=ContextMenuType.MESSAGE, name="UwUify", guild_ids=[865211274567483434]
)
async def slash_uwuify(ctx: MenuContext):
    await ctx.send(content=uwuify.uwu(ctx.target_message.content), hidden=False)
...
@slash.slash(
    ...
    options=[
        create_option(
            name="text", description="Text to UwUify", option_type=3, required=True
        )
    ],
)
...
async def slash_uploadschedule(ctx: SlashContext):
    ...
    if id in data["scheduledata"]:
        temp_msg = await ctx.send(
            f"Schedule for {ctx.author.mention} Already Found In DataBase. Do You Want To Update it?"
        )
        tick = "☑️"
        x = "❌"
        await temp_msg.add_reaction(tick)
...
async def slash_weekschedule(ctx: SlashContext, user=""):
    ...
    if id in data["scheduledata"]:
        ...
        if temp[0] != "Wrong Format":
            ...
            # await loadgif.delete()
            await loadgif.edit(
                content=f"Schedule for @{user.name}", file=discord.File(fp=fp)
            )
            os.remove(fp)
            ...
            aro = await ctx.guild.fetch_member(608276451074113539)
            await loadgif.edit(
                content=f"An error has occured While Retrieving your Data. Contact {aro.mention} with The following Time.\n\n`{dt.now()}`"
            )
    else:
        # await loadgif.delete()
        await loadgif.edit(
            content=f"Schedule For {user.mention} Not Found. Upload Your Schedule Using '{client.user.mention} uploadschedule' command"
        )
...
@slash.slash(
    ...
    options=[
        ...
        create_option(
            ...
            required=False,
            choices=[
                create_choice(name="Monday", value="mon"),
                create_choice(name="Tuesday", value="tue"),
                create_choice(name="Wednesday", value="wed"),
                create_choice(name="Thursday", value="thu"),
                create_choice(name="Friday", value="fri"),
            ],
        ),
    ],
)
async def slash_dailyschedule(ctx: SlashContext, user="", day="today"):
    ...
    if len(day) >= 3 and day[:3].lower() in (
        "mon",
        "tue",
        "wed",
        "thu",
    ...
    if temp[0] != "Wrong Format":
        ...
        img.save(fp)
        await loadgif.edit(
            content=f"Schedule for @{member.name}", file=discord.File(fp=fp)
        )
        os.remove(fp)
    else:
        aro = await ctx.guild.fetch_member(608276451074113539)
...
@slash.slash(
    ...
    options=[
        create_option(
            ...
            required=False,
            choices=[
                create_choice(name="uploadschedule", value="uploadschedule"),
                create_choice(name="weekschedule", value="weekschedule"),
                create_choice(name="dayschedule", value="dayschedule"),
                create_choice(name="countmein", value="countmein"),
                create_choice(name="addgif", value="addgif"),
                create_choice(name="listgifs", value="listgifs"),
                create_choice(name="fetchintro", value="fetchintro"),
                create_choice(name="scanschedule", value="scanschedule"),
            ],
        )
    ],
)
...
@slash.slash(
    ...
    options=[
        ...
        create_option(
            name="gif", description="Link to GIF", option_type=3, required=True
        ),
    ],
)
async def slash_addgif(ctx, keyword, gif):
    ...
    if keyword in commands:
        await ctx.send(
            f"`{keyword}` is a Command Used by the Bot. Try Again With a Different Keyword"
        )
        return
    elif keyword in data["gifs"] and mod not in author.roles:
        await ctx.send(
            f"Keyword `{keyword}` is in use. Request a Moderator to change the gif for the Designated keyword."
        )
        return
    if "https://" in gif and "gif" in gif:
        ...
        await logs_chn.send(
            embed=Embed(
                description=f"{ctx.author.mention} added a GIF to a keyword `{keyword}`"
            )
        )
        await client._after_invoke(ctx)
    else:
...
@slash.slash(
    ...
    guild_ids=[865211274567483434],
    options=[
        create_option(name="user", description="User", option_type=6, required=False)
    ],
)
async def slash_scanschedule(ctx: SlashContext, user=""):
    ...
    try:
        ...
        await loadgif.edit(
            content=f"Your schedule is not compatible with {client.mention}."
        )
        await logs_chn.send(
            embed=Embed(
                description=f"{ctx.author.mention} has faced an Error. [Message Link]({loadgif.jump_url})"
            )
        )
    # print(auth_data)
    mates = {}
    ...
    for member_id in data["scheduledata"]:
        ...
        try:
            ...
            await logs_chn.send(
                embed=Embed(
                    description=f"Error Occured While Scaning {client.get_user(member_id).mention}'s Schedule\n\n`{e}`"
                )
            )
        for course in member_data:
            for row in auth_data:
    ...
    if ctx.author.voice:
        ...
        if verified in ctx.author.roles:
            ...
            if cooldown > timedelta(minutes=5):
                await ctx.send(
                    " ".join([a.mention for a in ctx.author.voice.channel.members])
                )
                v_cooldown = dt.now(timezone("Asia/Kolkata"))
                await logs_chn.send(
                    embed=Embed(
                        description=f"{ctx.author.mention} pinged {len(ctx.author.voice.channel.members)} members in a VC"
                    )
                )
            else:
                s = cooldown.seconds
                await ctx.send(
                    f"This Comand is on Cooldown. Wait For {4 - (s//60)} mins {60-(s%60)} secs.",
                    hidden=True,
                )
        else:
