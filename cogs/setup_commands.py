...
class setup_commands(commands.Cog):
    ...
    async def setup(self ,ctx : commands.Context):
        ...
        kartus_category = await ctx.guild.create_category("kartus",overwrites=overwrites,position = 1)
        await kartus_category.edit(position=0)
        kartus_logs = await ctx.guild.create_text_channel("kartus-logs",category=kartus_category)
        kartus_deleted_messages = await ctx.guild.create_text_channel("deleted-messages",category=kartus_category)
        self.cursor.execute(f"""INSERT INTO guilds_info(guild_id,logs_channel_id,deleted_messages_channel_id) values( 
            '{ctx.guild.id}', 
            '{kartus_logs.id}', 
            '{kartus_deleted_messages.id}'
        )""")
        self.cursor.commit()
        await kartus_logs.send(embed = discord.Embed(description="Logs channel successfully created. This channel is currently only visible to the guild owner."))
    ...
    async def reset(self , ctx : commands.Context):
        ...
        if choice:
            self.cursor.execute(f"""DELETE FROM guilds_info WHERE guild_id = '{ctx.guild.id}'""")
            self.cursor.commit()
